...
class ADHDClassifier:
    ...
    def preprocess_data(self, X, y):
        ...
        if len(X.shape) == 2:
            if X.shape[1] == 95:
                X = X.reshape(X.shape[0], 19, 5)
                print("Reshaped data from (samples, 95) to (samples, 19, 5)")

        if len(X.shape) != 3 or X.shape[1] != 19 or X.shape[2] != 5:
            raise ValueError(f"Expected data shape (samples, 19, 5), got {X.shape}")

        X_normalized = np.zeros_like(X)
        for i in range(X.shape[0]):
            sample_flat = X[i].flatten()
            sample_normalized = self.scaler.fit_transform(sample_flat.reshape(-1, 1)).flatten()
            X_normalized[i] = sample_normalized.reshape(19, 5)
        X_final = X_normalized.reshape(X_normalized.shape[0], 19, 5, 1)
        
        print(f"Final preprocessed data shape: {X_final.shape}")
        return X_final, y
    ...
    def cross_validate(self, X, y, k=5):
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
            ...
            fold_model = self.build_cnn_model(input_shape)
    ...
    def save_model(self):
        ...
        model_path = os.path.join(self.model_save_folder, 'adhd_cnn_model_2d.h5')
...
def main():
    ...
    classifier = ADHDClassifier(data_folder='Przetwarzanie_bazy_danych/FurryV2', model_save_folder='Zapisana_siec')
