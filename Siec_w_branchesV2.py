...
class ADHDClassifier:
    ...
    def preprocess_data(self, X, y):
        ...
        delta_features = X_scaled[:, 0::5]   
        theta_features = X_scaled[:, 1::5]  
        alpha_features = X_scaled[:, 2::5] 
        beta_features = X_scaled[:, 3::5]  
        gamma_features = X_scaled[:, 4::5] 
        ...
        print(f"Gamma features shape: {gamma_features.shape}")

        delta_input = delta_features.reshape(delta_features.shape[0], delta_features.shape[1], 1)
        theta_input = theta_features.reshape(theta_features.shape[0], theta_features.shape[1], 1)
        alpha_input = alpha_features.reshape(alpha_features.shape[0], alpha_features.shape[1], 1)
        beta_input = beta_features.reshape(beta_features.shape[0], beta_features.shape[1], 1)
        gamma_input = gamma_features.reshape(gamma_features.shape[0], gamma_features.shape[1], 1)
        

        
        return [delta_input, theta_input, alpha_input, beta_input, gamma_input], y
    ...
    def build_cnn_model(self, input_shapes):
        delta_input = Input(shape=input_shapes[0], name='delta_input')
        theta_input = Input(shape=input_shapes[1], name='theta_input')
        alpha_input = Input(shape=input_shapes[2], name='alpha_input')
        beta_input = Input(shape=input_shapes[3], name='beta_input')
        gamma_input = Input(shape=input_shapes[4], name='gamma_input')
        def create_brainwave_branch(input_layer, name_prefix):
            x = Conv1D(32, 3, activation='relu', name=f'{name_prefix}_conv1')(input_layer)
            x = BatchNormalization(name=f'{name_prefix}_bn1')(x)
            x = MaxPooling1D(2, name=f'{name_prefix}_pool1')(x)
            x = Dropout(0.2, name=f'{name_prefix}_dropout1')(x)
            
            x = Conv1D(64, 3, activation='relu', name=f'{name_prefix}_conv2')(x)
            ...
            x = Dropout(0.3, name=f'{name_prefix}_dropout3')(x)
            
            return x
        delta_branch = create_brainwave_branch(delta_input, 'delta')
        theta_branch = create_brainwave_branch(theta_input, 'theta')
        alpha_branch = create_brainwave_branch(alpha_input, 'alpha')
        beta_branch = create_brainwave_branch(beta_input, 'beta')
        gamma_branch = create_brainwave_branch(gamma_input, 'gamma')

        combined = concatenate([delta_branch, theta_branch, alpha_branch, 
                               beta_branch, gamma_branch], 
        ...
        model = Model(inputs=[delta_input, theta_input, alpha_input, 
                             beta_input, gamma_input], 
                     outputs=output)
    ...
    def save_model(self):
        ...
        model_path = os.path.join(self.model_save_folder, 'adhd_cnn_model.h5')
...
def main():
    classifier = ADHDClassifier(data_folder='Przetwarzanie_bazy_danych/FurryV2', model_save_folder='Zapisana_siec_w_branchesV2')
    ...
    try:
        ...
        print(f"\nFinal data splits:")
        print(f"Training: {len(X_train[0])} samples")
        print(f"Validation: {len(X_val[0])} samples") 
        print(f"Test: {len(X_test_processed[0])} samples")
